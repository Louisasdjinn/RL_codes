...
datalist=[0,0] #refresh each episode to save the data
...
env = UnityEnvironment(base_port = 5004, worker_id = random.randint(0,7),file_name=env_name, side_channels = [engine_configuration_channel])
# 2. Load dependencies
...
for frame_idx in range(1, num_frames + 1):
    ...
    state = next_state
    step_result = env.get_step_result(group_name)
    #print("step",DQN.replay_buffer.step)
    #print("reward",reward)
    #print(all_rewards)
    if done or DQN.replay_buffer.step>=1000 :#TODO
        ...
        total_reward += reward
        i_ep = i_ep +1
        data_episode=[DQN.replay_buffer.step,reward]
        #step,reward,loss per epi
        datalist = np.vstack((datalist,data_episode))
        writedata(datalist)
        #if i_ep % 10 == 0:
        print("episodes {}, step is {} ".format(i_ep, DQN.replay_buffer.step))
        print("Total reward this episode: {}".format(total_reward))
        ...
        all_rewards.append(reward_episode)
        reward = 0
        reward_episode = 0
    ...
    if frame_idx % 1000 == 0:#whats mean TODO
